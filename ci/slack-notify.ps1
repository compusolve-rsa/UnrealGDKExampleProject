...
$slack_channel = Get-Env-Variable-Value-Or-Default -environment_variable_name "SLACK_CHANNEL" -default_value "#ken-debug-message"
$engine_version_count = "1"#buildkite-agent meta-data get "engine-version-count"
...
$gdk_commit_hash = "gdk_commit_hash"
...
# Start-Event "slack-notify" "slack-notify"
    ...
    Write-Output $slack_webhook_secret
    $slack_webhook_url = $slack_webhook_secret | ConvertFrom-Json | %{$_.url}
    Write-Output $slack_webhook_url
    ...
    if ($launch_deployment -eq "true") {
        for ($i = 0; $i -lt $engine_version_count; $i++){
            $deployment_name = "deployment-name-$($i+1)"
